...
public class LadderLength {
    ...
    public static int solution(String beginWord, String endWord, List<String> wordList) {
        ...
        // 所有字符的范围
        List<Character> range = getRange(wordList);
        ...
        while (!deque.isEmpty()) {
            ...
            for (int i = 0; i < curSize; i++) {
                ...
                for (int j = 0; j < beginWord.length(); j++) {
                    StringBuffer stringBuffer = new StringBuffer(curStr);
                    for (int k = 0; k < range.size(); k++) {
                        if (range.get(k).equals(stringBuffer.charAt(j))) {
                            continue;
                        }
                        stringBuffer.setCharAt(j, range.get(k));
                        String changed = stringBuffer.toString();
                        // 合法性
                        if (exist.contains(changed)) {
                            continue;
                        }
    ...
    private static List<Character> getRange(List<String> wordList) {
    ...
    // 每一个单词的编号
    public static Map<String, Integer> wordIdMap;
    // 全局计数
    public static int index;


    public static int solution2(String beginWord, String endWord, List<String> wordList) {
        edge = new ArrayList<>();
        ...
        addEdge(beginWord);
        // 找到图中的开始和结束的编号
        int beginIndex = wordIdMap.get(beginWord), endIndex = wordIdMap.get(endWord);
        Deque<Integer> deque = new LinkedList<>();
        deque.addLast(beginIndex);

        // dis[i] 代表 id为i的单词距离begin有多远
        int[] dis = new int[wordIdMap.size()];
        // 初始化
        ...
        while (!deque.isEmpty()) {
            ...
            if (cur == endIndex) {
                // 这里因为存在中间虚拟节点h*t的原因所以要/2
                return dis[endIndex]/2 + 1;
            }
            // 没到终点,去出来cur直接连接的节点列表
    ...
    private static void addEdge(String s) {
        addWord(s);
        // 取号
        Integer curIndex = wordIdMap.get(s);
        char[] chars = s.toCharArray();
        for (int i = 0; i < chars.length; i++) {
            char tmp = chars[i];
            // 每一个字母都为*一次
            ...
            // 加入编号
            addWord(change);
            Integer changeIndex = wordIdMap.get(change);
            // 互连
            edge.get(curIndex).add(changeIndex);
            edge.get(changeIndex).add(curIndex);
            chars[i] = tmp;
        }
    }
